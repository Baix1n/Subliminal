...
const defaultAffirmationCategories = {
  self: [
    "\u6211\u5f88\u5b89\u5168\uff0c\u6211\u4e5f\u5f88\u503c\u5f97\u88ab\u7231\u3002",
...
function selectedDefaultAffirmations() {
  const category = form.elements.affirmationCategory?.value || "theme";
  if (category !== "theme") return defaultAffirmationCategories[category] || defaultAffirmationCategories.self;
  const intentMode = form.elements.intentMode.value;
  const map = { general: "self", sp: "relationship", money: "money", study: "study" };
  return defaultAffirmationCategories[map[intentMode]] || defaultAffirmationCategories.self;
}
...
function getAffirmationText() {
  const intentMode = form.elements.intentMode.value;
  const defaults = selectedDefaultAffirmations();
...
function renderedDefaultAffirmations() {
  ...
  const selfName = form.elements.selfName.value.trim() || "我";
  const spName = form.elements.spName.value.trim() || "SP";
  return selectedDefaultAffirmations()
    .map((line) => line.replaceAll("{selfName}", selfName).replaceAll("{spName}", spName))
    .join("\n");
}
